...
$sql = "SELECT annotator_name, annotation_mode FROM Annotators WHERE annotator_name = ? AND password_md5 = ?";
$stmt= $conn->prepare($sql);
$stmt->bind_param("ss",  $name, $pw_md5);
$stmt->execute();
...
//    file_put_contents($file, "Annotator " . $id . " logged into the tool. " . $date.PHP_EOL, FILE_APPEND | LOCK_EX);
    $output =  array($credentials_match);
