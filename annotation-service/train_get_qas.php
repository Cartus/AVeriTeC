...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        array_push($gold_array, ["qa_id" => $row['qa_id'], "claim_norm_id" => $row['claim_norm_id'], "user_id_qa" => $row['user_id_qa'], "question" => $row['question'], "answer" => $row['answer'], "source_url" => $row['source_url'], "answer_type" => $row['answer_type'], 
        "source_medium" => $row['source_medium'], "answer_problems" => $row['answer_problems'], "bool_explanation" => $row['bool_explanation'], "answer_second" => $row['answer_second'], "source_url_second" => $row['source_url_second'], "answer_type_second" => $row['answer_type_second'], 
        "source_medium_second" => $row['source_medium_second'], "answer_problems_second" => $row['answer_problems_second'], "answer_problems_second" => $row['answer_problems_second'], "bool_explanation_second" => $row['bool_explanation_second'], "answer_third" => $row['answer_third'],
        "source_url_third" => $row['source_url_third'], "answer_type_third" => $row['answer_type_third'], "source_medium_third" => $row['source_medium_third'], "bool_explanation_third" => $row['bool_explanation_third'], "question_problems" => $row['question_problems'], 
        "qa_latest" => $row['qa_latest'], "phase_2_label" => $row['phase_2_label'], "correction_claim" => $row['correction_claim']]);
    }
} else {
    echo "0 Results";
}
...
//write json to file
if (file_put_contents("results/qas.json", $json))
