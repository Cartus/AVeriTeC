...
if ($req == "next-data"){
    ...
    $sql = "SELECT id, annotation_mode FROM Annotators WHERE id = ?";
    ...
    if($result->num_rows > 0){
        $sql = "SELECT id, refers_to, refers_to_fact_checking_site, claim_text, fact_checking_org, fact_checking_article_url, fact_checking_article_headline, fact_checking_verdict, fact_checking_date, original_claim_url, original_claim_source, fact_checking_article_raw_html) FROM $pages_table";
        $result = $conn->query($sql);
        $err = "Error description select data 1: " . $conn -> error;
        $row = $result->fetch_assoc(); // get first result
        $output =  array($row['id'], $row['refers_to'], $row['refers_to_fact_checking_site'], $row['claim_text'], $row['fact_checking_org'], $row['fact_checking_article_url'], $row['fact_checking_article_headline'], $row['fact_checking_verdict'], $row['fact_checking_date'], $row['original_claim_url'], $row['original_claim_source'], $row['fact_checking_article_raw_html']);
    }
...
//        die("Connection failed: " . $conn->connect_error);
//    }
//    $sql = "SELECT id FROM Annotators WHERE id = ?";
//    $stmt= $conn->prepare($sql);
//    $stmt->bind_param("i", $user_id);
//    $stmt->execute();
//    $result = $stmt->get_result();
