...
if ($result->num_rows > 0) {
    ...
    $average_task4_time =  round($row['p4_time_sum'] / max($row['finished_res_annotations'], 1), 2);
    ...
    $average_task4_load =  round($row['p4_load_sum'] / max($row['finished_res_annotations'], 1), 2);

    $average_questions_p2 = round($row['questions_p2'] / max($row['finished_qa_annotations'], 1), 2);
    $average_questions_p4 = round($row['questions_p4'] / max($row['finished_res_annotations'], 1), 2);
    ...
    $phase_4 = (["average_questions_p4" => $average_questions_p4, "annotations_done" => $row['finished_res_annotations'], "annotations_assigned" => $row['p4_assigned'], "annotations_timed_out" => $row['p4_timed_out'], "speed_traps_hit" => $row['p4_speed_trap'], "average_task_time" => $average_task4_time, "average_load_time" => $average_task4_load]);
